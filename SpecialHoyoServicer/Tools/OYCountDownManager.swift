...
class OYCountDownManager: NSObject {
    ...
    func start() ->Void {
      //  weak var weakSelf = self
        self.timer =  Timer(timeInterval: 1, target: self, selector: #selector(timerAction), userInfo: nil, repeats: true)
//        Timer.scheduledTimer(withTimeInterval: 1, repeats: true, block: { (timers) in
