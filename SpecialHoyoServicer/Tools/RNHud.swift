...
struct RNHud {
    ...
    private var loadingGroup: STLoadingGroup?

    // 初始化 loadingGroup, 使用 STLoadingGroup
  
    init(superView: UIView, side: CGFloat = 50.0, style: STLoadingStyle = .submit) {
        
        loadingGroup = STLoadingGroup(side: side, style: style)
            
        loadingGroup?.show(superView)
    }
    ...
    func stopLoading(callBack: (() -> ())? = nil) {
        
        if let callback = callBack {
             loadingGroup?.stopLoading(finish: callback)
        }else{
