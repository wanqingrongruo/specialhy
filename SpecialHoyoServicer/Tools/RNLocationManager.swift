...
class RNLocationManager: NSObject{
    ...
    var detialAddress: ((_ address: String) -> Void)? // 返回详细地址...
...
extension RNLocationManager {
    ...
    public func getAddress(latitude: Double, longitude: Double) {
        ...
        geocoder.reverseGeocodeLocation(location) { (placemarks, error) in
            ...
            if self.detialAddress != nil {
                if let city = place.locality {  }// 直辖市其值为 nil
                
                var s = (place.administrativeArea ?? "")
                s += (place.locality ?? "")
                s += (place.subLocality ?? "")
                s += (place.name ?? "")
                let ad = s
                self.detialAddress?(ad)
            }
