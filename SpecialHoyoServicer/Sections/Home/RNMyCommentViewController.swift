//
//  RNMyCommentViewController.swift
//  SpecialHoyoServicer
//
...
class RNMyCommentViewController: RNModelTableViewController {
    ...
    lazy var dataSource: [Order] = {
        ...
        let array = realmQueryModel(Model: Order.self, condition: { (results) -> Results<Order>? in
            ...
            return results.filter("state = 5") // 从数据库中拉取已评价订单
        })
...
//MARK: - custom methods
extension RNMyCommentViewController {
    ...
    func parameters() {
        ...
        parameter["actiontype"] = OrderType.commented.rawValue
...
extension RNMyCommentViewController: UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        completedDetailVC.isComment = true
...
extension RNMyCommentViewController: MapViewControllerProtocol{
    
    func showMapView(destinationLocation: CLLocationCoordinate2D, destinationString: String) {
        
        let mapVC = RNMapViewController.init("我的评价", destinationLocation: destinationLocation, destinationString: destinationString)
