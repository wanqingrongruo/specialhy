//
//  RNSubscribeViewController.swift
//  SpecialHoyoServicer
//
...
class RNSubscribeViewController: RNBaseViewController {
    ...
    lazy var dataSource: [Order] = {
        ...
        let array = realmQueryModel(Model: Order.self, condition: { (results) -> Results<Order>? in
            ...
            return results.filter("state = 2") // 从数据库中拉取 待处理订单
        })
...
//MARK: - custom methods
extension RNSubscribeViewController {
...
extension RNSubscribeViewController: UITableViewDelegate, UITableViewDataSource{
...
extension RNSubscribeViewController: RNActionSheetProtocol, MapViewControllerProtocol, RecordProtocol{
    ...
    func showMapView(destinationLocation: CLLocationCoordinate2D, destinationString: String) {
        ...
        let mapVC = RNMapViewController.init("已预约", destinationLocation: destinationLocation, destinationString: destinationString)
