...
class RNSecondSubmitViewController: RNBaseTableViewController {
...
// MARK: - event response
extension RNSecondSubmitViewController {
...
extension RNSecondSubmitViewController {
    ...
    func setUpDatas() {
        ...
        let threes = ["", "配件使用", "费用统计","支付方式"] // 第一行显示图片 //["","费用明细"]  //
    ...
    func setUI() {
        ...
        bottomView?.callBack = { [weak self] tag in
            // 提交
            self?.bottomView?.sigleButton.isEnabled = false
            self?.bottomView?.sigleButton.backgroundColor = UIColor.lightGray
            self?.submitToServer()
        }
    ...
    func skipToNewFeeCaculateVC(_ cell: RNSkipCell) {
        ...
        //                tmp += Double(item.productPrice ?? "0.00") ?? 0.00
        //            }
...
extension RNSecondSubmitViewController {
    ...
    func doMoneylist() -> [[String: Any?]] {
        var arr = [[String: Any?]]()
        for item in feeArray {
            var dic = [String: Any?]()
            dic["title"] = Array(item.keys).first
            dic["money"] = Array(item.values).first
            arr.append(dic)
        }
    ...
    func mergeParam() -> Bool{
        ...
        if currentType == 6 {
            ...
            guard let boxState = boxState else {
                RNNoticeAlert.showError("提示", body: "请选择开箱状态")
                return false
            }
            
            if boxState == "不良" {
                guard let _ = parameters["Remark"] else {
                    RNNoticeAlert.showError("提示", body: "当开箱状态不良时必须填写备注描述")
                    return false
                }
            }
    ...
    func activeCompletedButton() {
        self.bottomView?.sigleButton.isEnabled = true
        self.bottomView?.sigleButton.backgroundColor = MAIN_THEME_COLOR
    }
    ...
    func submitToServer(){
        ...
        RNHud().showHud(nil)
        OrderServicer.finishedOrder(["orderFinishInfo": orderInf, "usertoken": token], imageDatas: imageParams, successClourue: { [weak self](state) in
            RNHud().hiddenHub()
...
extension RNSecondSubmitViewController: UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            if indexPath.section == 1 {
                ...
                }else if currentType == 4 || currentType == 14{ // 充水
...
extension RNSecondSubmitViewController: RNSubmitCustomCellProtocol {
...
extension RNSecondSubmitViewController: RNSubmitWithoutQRCellProtocol{
...
extension RNSecondSubmitViewController: RNLeftWaterCellProtocol{
...
extension RNSecondSubmitViewController: RNBoxStateCellProtocol{
...
extension RNSecondSubmitViewController: RNImageCellProtocol{
