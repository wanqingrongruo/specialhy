//
//  RNCompletedViewController.swift
//  SpecialHoyoServicer
//
...
class RNCompletedViewController: RNBaseViewController {
    ...
    lazy var dataSource: [Order] = {
        ...
        let array = realmQueryModel(Model: Order.self, condition: { (results) -> Results<Order>? in
            ...
            return results.filter("state = 3") // 从数据库中拉取 待处理订单
        })
    ...
    func addObserveAction() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(notificationAction), name: NSNotification.Name(rawValue: PaySuccessNotification), object: nil) // 支付成功
    }
...
//MARK: - custom methods
extension RNCompletedViewController {
...
//        }
    ...
    func parameters() {
        ...
        parameter["actiontype"] = OrderType.completed.rawValue
...
extension RNCompletedViewController: UITableViewDelegate, UITableViewDataSource{
...
extension RNCompletedViewController: MapViewControllerProtocol{
    
    func showMapView(destinationLocation: CLLocationCoordinate2D, destinationString: String) {
        
        let mapVC = RNMapViewController.init("已完成", destinationLocation: destinationLocation, destinationString: destinationString)
