...
- (void)updateNavAttributesWithViewController:(UIViewController *)viewController
    ...
    if (_sdkConfig.presentingAnimation == UDTransiteAnimationTypePresent) {
        ...
        // 自定义取消按钮, 不用系统的取消按钮 --- 我的代码 by roni
        UIBarButtonItem * item = [[UIBarButtonItem alloc] initWithTitle:@"取消" style: UIBarButtonItemStylePlain target:viewController action:@selector(dismissChatViewController)];
